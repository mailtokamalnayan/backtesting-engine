...
Once `vercel` is installed and authenticated (`npm i -g vercel && vercel login`),
one command takes the current saved runs live:
...
forgotten one: a `*.vercel.app` alias can't be a project production domain, so a
clean alias does **not** auto-follow a prod deploy and must be re-pointed every time.

Set the target in your `.env` (see `.env.example`): `VERCEL_SCOPE` (your team/account
slug) and `VERCEL_ALIAS` (the clean alias to re-point). Both are optional — with no
scope the deploy uses your Vercel default; with no alias the alias step is skipped and
the immutable deploy URL is what goes live.
